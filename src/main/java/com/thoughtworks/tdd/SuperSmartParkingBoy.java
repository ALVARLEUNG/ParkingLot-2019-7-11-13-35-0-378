...
public class SuperSmartParkingBoy extends ParkingBoy{
    ...
    public ParkingCarResult parkCarInTheLot(Car car) {
        ...
        ParkingLot parkingLot = this.getParkingLots().stream().reduce((item, items) -> (double)(1-item.getParkingCarTicket().size()/item.getLimit()) > (double) (1-items.getParkingCarTicket().size()/items.getLimit()) ? item : items).orElse(new ParkingLot());
